...
void LexicalAnalyser::tokenize(std::vector<std::string>& list)
{
	...
	{	
		...
		if (_previousChar == '\n' || _previousChar == '\r')
		{
			if (_linesIt != _lines.end() && (*_linesIt).size() == 0)
			{
				...
				else
				{
					...
					else
					{
						if (_tokenCollection.isStringBreakingChar(_nextChar))
						{
							...
							const Token search = _tokenCollection.isToken(word);
							const Token charSearch = _tokenCollection.isToken(std::string("") + _nextChar);
		...
		else if (_taggerTools.isNum(_previousChar))
		{
			...
			const Token charSearch = _tokenCollection.isToken(std::string("") + _previousChar);
			if (charSearch.tokenValue == ";")
			{
...
TaggedLexeme::TaggedLexeme(const Token& t, const int l) : taggedWord(t), line(l)
{
}

void LexicalAnalyser::_addToOutput(const Token& token)
{
	_output.push_back(TaggedLexeme(token, _iLine));
}

std::vector<TaggedLexeme> LexicalAnalyser::getOutput()
{
	return _output;
}
